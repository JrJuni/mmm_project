...
- 추적 키워드(`MMM_KEYWORD` 환경변수로 설정)의 국가별 검색 관심도를 SerpApi를
  통해 Google Trends에서 수집합니다.
...
SerpApi 무료 플랜 기준: **월 250회**(매월 갱신), **시간당 50회** 처리량
([가격 페이지](https://serpapi.com/pricing), 2026-06-24 확인). 사용 전 최신
약관을 직접 확인하세요. [SerpApi 약관](https://serpapi.com/legal)도 참고.

- **상업적 사용:** 무료 플랜을 비상업용으로 제한하는 조항은 없습니다(무료티어
  전용 제외 항목은 아래 Legal Shield뿐). 받은 데이터를 자사 제품에 활용하는 건
  허용됩니다. 확실히 하려면 `support@serpapi.com`에 문의하세요.
- **원시 데이터 재판매 금지:** 자사 제품 *안에서* 쓰는 건 되지만, 약관(Section
  2)은 raw 검색 결과를 제3자에 재판매·재배포하는 것을 금지합니다. 이 프로젝트는
  raw 데이터를 외부로 내보내지 않습니다 — MCP는 읽기전용, 정제된 구조화 필드만
  반환.
- **캐싱은 문제없음:** `data/data.json` 저장은 허용되며 보관 기간 제한 조항이
  없습니다. (별개로 SerpApi는 동일 쿼리를 서버측에서 ~1시간 캐시, 무료 처리.)
- **무료/스타터/디벨로퍼엔 Legal Shield 없음:** 공개 데이터의 합법적 *수집*에
  대한 최대 $2M 법적 보호는 상위 유료 플랜에만 적용되며, 데이터를 *어떻게
  쓰는지*는 보장하지 않습니다. 공개 집계 통계의 내부 활용은 실질 리스크가 낮음;
  규모 확대·외부 노출 시 재검토.
